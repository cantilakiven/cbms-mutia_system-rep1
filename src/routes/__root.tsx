...
// Separate component for protected view to ensure React DOES NOT mount AppShell until unlocked
...
function SecureLoginGate() {
    ...
    void (async () => {
      try {
        ...
        if (state?.storageError) {
          setStorageError(String(state?.error || "Security storage could not be verified."));
        }
      } catch (e) {
        if (mounted) {
          setStorageError("Failed to communicate with local security service.");
        }
      } finally {
        if (mounted) {
          setChecking(false);
      ...
      <div className="security-gate relative min-h-screen overflow-hidden bg-slate-950 p-6 grid place-items-center">
        ...
        <main className="relative z-10 flex flex-col items-center justify-center p-8 text-center">
          <div className="relative flex items-center justify-center">
            ...
            <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl border border-white/20 bg-slate-900/80 shadow-2xl backdrop-blur-md">
          ...
          <div className="mt-6 h-1 w-48 overflow-hidden rounded-full bg-slate-800">
            <div className="h-full w-full animate-[shimmer_1.5s_infinite] bg-gradient-to-r from-transparent via-cyan-400 to-transparent -translate-x-full" />
          </div>
        </main>
      </div>
    );
  ...
  // 2. Storage/Decryption Error -> Block access completely
  if (storageError) {
    return (
      <div className="min-h-screen grid place-items-center bg-slate-950 p-6 text-white">
        <main className="w-full max-w-lg rounded-[28px] border border-red-400/20 bg-slate-900 p-7 shadow-2xl">
          <div className="flex items-center gap-4">
            <img src={logo} alt="Selected Local Area" className="h-16 w-16 rounded-2xl bg-white p-2" />
            <div>
              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-red-300">Security storage error</div>
              <h1 className="mt-1 text-2xl font-black">Access blocked</h1>
            </div>
          </div>
          <p className="mt-5 text-sm leading-6 text-slate-300">{storageError}</p>
          <p className="mt-3 text-xs leading-5 text-slate-400">
            Close the application completely and start it again. The application will not open its data until the existing security record can be verified.
          </p>
        </main>
      </div>
    );
  ...
  // 3. Unlocked or Security Disabled -> Only NOW load protected app components into DOM
      ...
      <main className="security-gate-card relative z-10 w-full max-w-md overflow-hidden rounded-[30px] border border-white/25 bg-slate-950/70 p-7 shadow-[0_30px_100px_rgba(0,0,0,.45)] backdrop-blur-xl">
        ...
        <div className="relative flex items-center gap-4">
          <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl border border-white/20 bg-white/95 p-2 shadow-xl">
            <img src={logo} alt="Selected Local Area" className="h-full w-full rounded-xl object-contain" />
          </div>
          <div>
            <div className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-300">Protected access</div>
            <h1 className="mt-1 font-display text-2xl font-black text-white">CBMS Insights</h1>
            <p className="text-xs text-slate-300">Community-Based Monitoring System</p>
          </div>
        </div>
        <div className="relative mt-7 overflow-hidden rounded-2xl border border-white/15 bg-white/8 p-4">
          ...
          <div className="flex items-center gap-2 text-sm font-bold text-white">
            <ShieldCheck className="h-4 w-4 text-cyan-300" /> Secure system
          </div>
        ...
        <button
          ...
          disabled={lockedUntil > Date.now()}
          onClick={() => void submit()}
          className="relative mt-4 flex w-full disabled:cursor-not-allowed disabled:opacity-50 items-center justify-center gap-2 rounded-2xl border border-cyan-200/20 bg-cyan-500/90 px-4 py-3.5 text-sm font-black text-slate-950 shadow-xl shadow-cyan-950/30 transition hover:bg-cyan-300"
        >
          <LockKeyhole className="h-4 w-4" /> Unlock Secure System
        </button>
        {lockedUntil > Date.now() ? (
          ...
          <p className="relative mt-3 text-center text-sm font-semibold text-red-300">
            {error || `${attemptsRemaining} attempt${attemptsRemaining === 1 ? "" : "s"} remaining.`}
          </p>
        )}
        <div className="relative mt-5 flex items-center justify-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,.7)]" /> Local secure session <span className="mx-1 text-slate-600">•</span> Selected Local Area
        </div>
      </main>
    </div>
  );
