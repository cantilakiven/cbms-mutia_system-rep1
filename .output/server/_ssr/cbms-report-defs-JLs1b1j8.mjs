function householdSizeBucket(n) {
...
function frequency(rowsIn, report) {
  ...
  const rows = Array.from(counts.entries()).map(([category, count]) => ({ category, count, percent: total ? `${(count / total * 100).toFixed(2)}%` : "0%" })).sort((a, b) => b.count - a.count);
  return { rows, total };
}
export {
  REPORTS as R,
